...
// Fibonacci action messages
//go:generate gengo -out=$GOPATH/src action actionlib_tutorials/Fibonacci
import (
	"actionlib_tutorials"
	"fmt"
	"os"
...
func newFibonacciServer(node ros.Node, name string) {
	...
	s.as = actionlib.NewSimpleActionServer(node, name, actionlib_tutorials.ActionFibonacci, s.executeCallback, false)
	s.as.Start()
}
